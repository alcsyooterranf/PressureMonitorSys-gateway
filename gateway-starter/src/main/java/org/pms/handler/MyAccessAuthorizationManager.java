...
public class MyAccessAuthorizationManager implements AuthorizationManager<RequestAuthorizationContext> {
	...
	public AuthorizationDecision check(Supplier<Authentication> authentication, RequestAuthorizationContext object) {
		...
		else if (uri.startsWith("/product_manage") && authentication.get().getAuthorities().stream().anyMatch(
				authority -> "ROLE_admin".equals(authority.getAuthority()) ||
						"ROLE_operator".equals(authority.getAuthority()))) {
			return new AuthorizationDecision(true);
		}
		// 配置异常上报模块权限, 允许admin和operator访问
		else if (uri.startsWith("/abnormal_report") && authentication.get().getAuthorities().stream().anyMatch(
				authority -> "ROLE_admin".equals(authority.getAuthority()) ||
						"ROLE_operator".equals(authority.getAuthority()))) {
			return new AuthorizationDecision(true);
		}
		// 配置指令下发模块权限, 只允许admin访问
